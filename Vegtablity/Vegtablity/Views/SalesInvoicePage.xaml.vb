...
Namespace Views
    Public Class SalesInvoicePage
        ...
        Private Sub PartnerComboBox_LostFocus(sender As Object, e As RoutedEventArgs)
            Dim cmb = TryCast(sender, ComboBox)
            If cmb Is Nothing Then Return
            ' إعادة ضبط الفلتر عند مغادرة الـ ComboBox
            Dim vm = TryCast(Me.DataContext, SalesInvoiceViewModel)
            If vm IsNot Nothing Then
                vm.PartnerSearchText = ""
                ' إعادة عرض اسم الشريك المحدد
                If cmb.SelectedItem IsNot Nothing Then
        ...
        Private Sub dgInvoiceDetails_ArrowNav_PreviewKeyDown(sender As Object, e As KeyEventArgs)
            ...
            ' فقط عند وجود خلية في وضع التعديل
            ...
            Select Case e.Key
                ...
                Case Key.Right : If colIndex > 0 Then targetCol -= 1 Else Return        ' RTL
                Case Key.Left  : If colIndex < dg.Columns.Count - 1 Then targetCol += 1 Else Return  ' RTL
                Case Else      : Return
            End Select
        ...
        Private Sub PartnerComboBox_PreviewKeyDown(sender As Object, e As System.Windows.Input.KeyEventArgs)
            If e.Key = System.Windows.Input.Key.Enter Then
                ...
                Dim vm = TryCast(Me.DataContext, ViewModels.SalesInvoiceViewModel)
                If vm Is Nothing Then Return
